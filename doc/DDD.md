...
# 第二阶段设计（2026-06-25 新增）

> 对应 [PRD.md](PRD.md)「第二阶段需求」R1–R9，分阶段计划见 [plan/02plan.md](plan/02plan.md)。
...
- **开启方式**：`reasoning_effort="high"`（复杂可 `"max"`）+ `extra_body={"thinking": {"type": "enabled"}}`，调用时按需传入；需用**推理型模型**（如 `deepseek-v4-pro`，一期默认 `deepseek-v4-flash` 不带推理）。`config.py` 增 `REASONING_MODEL` / `REASONING_EFFORT`；CLI `:think` 开关写入 `RunContext.reasoning`。
...
- `config.py`：`REASONING_MODEL`、`REASONING_EFFORT`、`LOG_DIR`、`LOG_NAME_MAXLEN`、`DANGER_PATTERN`、`BASH_TIMEOUT`、`FETCH_TIMEOUT`。
