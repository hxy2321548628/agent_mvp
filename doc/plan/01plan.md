...
|---|---|
| 目标 | 项目可跑起来、依赖齐、密钥可读 |
| 任务 | 补依赖（`pytest`、`pytest-cov`）；按 [DDD §3](../ddd/01ddd.md) 建空包骨架；`config.py`：顶层参数（`DEFAULT_MODEL=deepseek-v4-pro`、`DEFAULT_BASE_URL=https://api.deepseek.com`、`MAX_TURN`、`MAX_MSG`、`KEEP_RECENT`、`STREAM`）+ `Settings`（读 `.env`：`DEEPSEEK_API_KEY` 等）；`.env.example` |
