...
```
SessionPrefix → Log →[Record]→ Trace → MaxTurn → Context → Approval → Retry
```

| 顺序 | 中间件 | 主要钩子 | 为什么在这个位置 |
|---|---|---|---|
| 1 | **SessionPrefix** | `on_session_start` | 前缀要最先装配好，后续阶段（压缩、模型调用）才看得到完整的系统提示 |
| 2 | **Log** | 顺序钩子 | 紧跟前缀、早注册，以便完整记录每轮事件成机读运行日志（累积 `ctx.run_log`，`persist` 落 JSONL；[10 §10.2](10-evaluation.md)） |
...
> `[Record]`（在线录制）是可选中间件：仅当 CLI 用 `:cassette` 录制时插在 Log 之后；`Trace`/`Context` 与 Log 的落盘（`persist`）同为开关项（`eval` 关 I/O、单轮不压缩，但 Log 仍累积 `ctx.run_log` 供评测）。行为核心恒在且同序，差异收敛为 `build_middlewares` 的几个具名开关（见 [10 §10.4/§10.6](10-evaluation.md)）。
