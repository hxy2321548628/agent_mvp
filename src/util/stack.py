...
行为核心恒在且同序——SessionPrefix → Log →[Record]→[Trace]→ MaxTurn →[Context]→ Approval → Retry；
Log 常在（累积 ctx.run_log 供 eval），仅 persist 开关决定是否落盘；差异收敛为几个显式开关
（确认函数、Log 是否落盘、是否带 Trace、是否带 Context），而非两份手维护的列表。
...
def build_middlewares(
    *,
    ...
    """组装默认中间件栈。cli 全开（log 落盘/trace/context + 交互确认 + 可选录制），eval 关 I/O、自动放行、单轮不压缩。"""
    middlewares: list[Middleware] = [
        SessionPrefixMiddleware(todo=todo_store, env=build_runtime_env(settings)),
        LogMiddleware(log_dir=LOG_DIR, name_maxlen=LOG_NAME_MAXLEN, model=model, persist=log),
    ]
